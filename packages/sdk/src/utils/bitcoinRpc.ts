...
export class BitcoinRPC {
  ...
  constructor({ network, bitcoinRpc }: { network: bitcoin.Network; bitcoinRpc: string }) {
    if (bitcoinRpc === 'mempool') {
      this.apiUrl = network === bitcoin.networks.testnet ? MempoolRpcUrl['testnet'] : MempoolRpcUrl['mainnet'];
    } else {
      this.apiUrl = bitcoinRpc;
    }

    this.feeApiUrl = network === bitcoin.networks.testnet ? MempoolRpcUrl['testnet'] : MempoolRpcUrl['mainnet'];
  }
  ...
  async getUTXOs(address: string): Promise<UTXO[]> {
    try {
      const response: AxiosResponse<UTXO[]> = await axios.get(`${this.apiUrl}/address/${address}/utxo`);
  ...
  async getRawTransaction(txid: string): Promise<string> {
    try {
      const response: AxiosResponse<string> = await axios.get(`${this.apiUrl}/tx/${txid}/hex`);
      return response.data;
    } catch (error) {
  ...
  async broadcast(hex: string): Promise<string> {
    try {
      const response: AxiosResponse<string> = await axios.post(`${this.apiUrl}/tx`, hex, {
        headers: {
