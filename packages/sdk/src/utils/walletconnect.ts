import type { SupportedETHSigningMethods, PKPEthersWallet } from '@lit-protocol/pkp-ethers';
import { ethRequestHandler, methodHandlers, isEthRequest } from '@lit-protocol/pkp-ethers';
import { LIT_CHAINS } from '@lit-protocol/constants';
import { Core } from '@walletconnect/core';
import type { IWeb3Wallet, Web3WalletTypes } from '@walletconnect/web3wallet';
import { Web3Wallet } from '@walletconnect/web3wallet';
import { formatAccountWithChain, getSdkError, parseChainId } from '@walletconnect/utils';
import type { ErrorResponse, JsonRpcResponse } from '@walletconnect/jsonrpc-utils';
import { formatJsonRpcError, formatJsonRpcResult } from '@walletconnect/jsonrpc-utils';
import type { CoreTypes, ISignClient, SessionTypes, SignClientTypes } from '@walletconnect/types';

const DEFAULT_RELAY_URL = 'wss://relay.walletconnect.com';

export interface InitWalletConnectParams extends Omit<Web3WalletTypes.Options, 'core'> {
  projectId: string;
  relayUrl?: string;
}
...
export class PKPWalletConnect {
  ...
  // List of PKPClients
  private pkpClients: PKPEthersWallet[] = [];
   ...
   */
  public async initWalletConnect(params: InitWalletConnectParams): Promise<void> {
    if (!params.projectId) {
  ...
  public async approveSessionProposal(
    ...
    for (const key of requiredNamespaceKeys) {
      ...
      if (chains) {
        for (const chain of chains) {
          ...
          const supportedMethods = this.filterUnsupportedMethods(requiredNamespaces[key].methods);
          if (requiredNamespaces[key].methods.length !== supportedMethods.length) {
            const unsupportedMethods = requiredNamespaces[key].methods.filter(
              (method) => !supportedMethods.includes(method)
            );
            return await this.client.rejectSession({
              id,
              reason: getSdkError('UNSUPPORTED_METHODS', `Unsupported methods: ${unsupportedMethods.join(', ')}`),
            });
          }
    ...
    for (const key of optionalNamespaceKeys) {
      ...
      const optionalNamespaceSupportedMethods = this.filterUnsupportedMethods(optionalNamespaces[key].methods);
      if (!namespaces[key]) {
        namespaces[key] = {
          accounts,
          chains: key.includes(':') ? [key] : chains,
        ...
        namespaces[key].accounts = [...new Set([...namespaces[key].accounts, ...accounts])];
        namespaces[key].chains = [...new Set([...(namespaces[key].chains || []), ...(chains || [])])];
        namespaces[key].methods = [...new Set([...namespaces[key].methods, ...optionalNamespaceSupportedMethods])];
        namespaces[key].events = [...new Set([...namespaces[key].events, ...optionalNamespaces[key].events])];
      }
    }
   ...
   */
  public rejectSession: IWeb3Wallet['rejectSession'] = async (params: { id: number; reason: ErrorResponse }) => {
    this.client = this._isWalletConnectInitialized(this.client);
   ...
   */
  public async approveSessionRequest(requestEvent: SignClientTypes.EventArguments['session_request']): Promise<void> {
    this.client = this._isWalletConnectInitialized(this.client);
  ...
  public async rejectSessionRequest(
    ...
    const response = formatJsonRpcError(id, reason || getSdkError('USER_REJECTED'));
    return await this.respondSessionRequest({
      topic,
      response,
    });
   ...
   * @returns { Promise<void> }
   */
  public extendSession: IWeb3Wallet['extendSession'] = async (params: { topic: string }): Promise<void> => {
    this.client = this._isWalletConnectInitialized(this.client);
  ...
  public removeListener(name: Web3WalletTypes.Event, listener: (args: any) => void) {
    this.client = this._isWalletConnectInitialized(this.client);
  ...
  public async getAccounts(chainName: string): Promise<string[]> {
    ...
    for (const pkpClient of this.pkpClients) {
      ...
      switch (chainName) {
        case 'eip155':
          address = await pkpClient.getAddress();
   ...
   */
  public async findPKPClientByRequestParams(params: any): Promise<PKPEthersWallet | null> {
    const paramsString = JSON.stringify(params);

    // Loop through all wallets and find the one that has an address that can be found within the request params
    for (const pkpClient of this.pkpClients) {
      // TODO: Update this once we support more JSON RPC handlers
      const ethAddress = await pkpClient.getAddress();
   ...
   */
  public addPKPClient(pkpClient: PKPEthersWallet): void {
    const existingClient = this.pkpClients.find((client) => client.getAddress() === pkpClient.getAddress());
    if (!existingClient) {
   ...
   */
  public getPKPClients(): PKPEthersWallet[] {
   ...
   */
  public setPKPClients(clients: PKPEthersWallet[]): void {
   ...
   */
  private _isWalletConnectInitialized(client: IWeb3Wallet | undefined): IWeb3Wallet {
    if (!client) {
      this._log('WalletConnect client has not yet been initialized.');
      return this._throwError('WalletConnect client has not yet been initialized. Please call initWalletConnect().');
    }
    return client;
  }
